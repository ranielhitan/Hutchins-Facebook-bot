 ...
 onLaunch: async function({ api, event, target }) {
  ...
  if (!text && name) {
    var data = fs.readFile(
      ...
      "utf-8",
      async (err, data) => {
        if (err) return api.sendMessage(`Command ${target[0]} does not exist!.`, threadID, messageID);
  ...
  if (url[0].indexOf('pastebin') !== -1) {
    axios.get(url[0]).then(i => {
      ...
      fs.writeFile(
        ...
        "utf-8",
        function(err) {
          if (err) return api.sendMessage(`An error occurred while applying the code ${target[0]}.js`, threadID, messageID);
          api.sendMessage(`Code applied ${target[0]}.js, use command load to use!`, threadID, messageID);
        }
      );
    })
  }
    ...
    request(options, function(error, response, body) {
      ...
      load('.language-js').each((index, el) => {
        ...
        fs.writeFile(`${__dirname}/${target[0]}.js`, code, "utf-8",
          function(err) {
            if (err) return api.sendMessage(`An error occurred while applying the new code to "${target[0]}.js".`, threadID, messageID);
            return api.sendMessage(`Added this code "${targt[0]}.js", use command load to use!`, threadID, messageID);
          }
        );
      });
    });
  ...
  if (url[0].indexOf('drive.google') !== -1) {
    ...
    try {
      ...
      return api.sendMessage(`An error occurred while applying the new code to "${target[0]}.js".`, threadID, messageID);
    }
  }
}
  }